...
> 当前源码版本为 **1.3.0 候选版**，只面向 **Intel Mac（x86_64）+ macOS Sonoma 14 或更高版本**。已知目标机是 2019 款 Intel MacBook Air。正式 Release 需通过该设备验收；Apple Silicon 暂不在本版支持范围内。

## 1.3 修复

- 部分来源失败但仍有新文章时，首页展示这批可用结果并标注“部分完成”；下次去重不会让它们从首页消失。没有新增、取消或全部失败时继续显示上一份可用报告和两次时间。
- 去重保留小数、百分比、单位、月份和政策阶段的实质变化；`1.5% → 15%`、`June → July` 等更新不再误删。
- 同一数据目录只能运行一个后端。Mac 重复打开会打开已有驾驶舱；崩溃后下一次启动可以安全恢复中断状态。
- 编辑主题保留已停用/已移除来源的原绑定。移除最后一个来源不会自动改抓所有网站；需要明确调整该主题的来源。
- 安全导入保留本机已移除来源与修改后的预设地址。旧版删除造成范围歧义时，驾驶舱提示检查对应主题。
- 来源验证按钮的等待时间覆盖后台处理预算；任务完成、失败、取消后刷新报告、来源错误和连接状态。
- 同秒连续任务及跨机导入保持正确先后顺序；损坏或未写完的报告不会遮蔽上一份可用结果。安全合并历史不会抢占本机首页报告。

仍沿用核心＋补充阅读、20＋15 语义预算、白底 Cookie 视觉、schema v2 备份和本地数据目录。

验收范围、已知限制与真机操作清单见 [1.3 候选版验收记录](docs/release-1.3.0.md)。
...
- 首次打开可按类别选择一套中英文 RSS 来源；跳过向导时使用 4 个平衡来源。内置目录共 50 个订阅，银行与金融来源包新增 26 个：央行、银行监管、行业动态和金融科技。全部新增默认关闭，升级不改变原启停、编辑或来源绑定。来源页支持分类与搜索，详见 [银行来源目录与验证记录](docs/source-catalog-v3.md)。
...
- 1.3 使用 `deepseek-v4-flash` 的非思考模式完成结构化筛选、摘要与主题建议；模型名称会在驾驶舱中明确显示。
...
1.3.0 没有 Apple Developer 证书，因此没有经过 Apple 公证。构建过程会做 **ad-hoc 完整性签名** 并验证签名，但这不等同于开发者签名或 Apple 公证。macOS 第一次运行仍会要求使用者手动批准，这是预期行为，不是安装失败。
...
3. 点 **Run workflow**，输入与源码一致的版本号（当前为 `1.3.0`）并运行。
...
2. 在 **Choose a tag** 中输入与源码版本对应的标签，例如 `v1.3.0`，选择 **Create new tag: v1.3.0 on publish**；目标分支选 `main`。
3. 标题可填 `Cookies News Cockpit 1.3.0`。此时不要手动上传从 Actions 下载的外层 ZIP。
4. 点 **Publish release**。GitHub 创建标签后会自动触发 **Build Intel macOS DMG**；刚发布的 Release 可能暂时还没有安装文件，这是正常的。
5. 回到 **Actions**，等待这次由 `v1.3.0` 标签触发的运行变为绿色。发布步骤会识别网页端已存在的同名 Release，覆盖同名资产并更新标准标题和说明，不会因为 Release 已存在而冲突失败。
...
正式 Release 资产会一直保留到维护者主动删除，并提供每个文件的直接下载入口；Actions artifact 仍只保留 30 天。推送形如 `v1.3.0` 的 Git tag 也会走同一条正式发布流水线。
...
```bash
APP_VERSION=1.3.0 bash packaging/build_macos.sh
```
...
- **1.1 或 1.2 → 1.3：**可直接覆盖升级。首次启动先生成 `backups/before-upgrade-1.3.0.sqlite3`，已有 1.2 升级快照不会覆盖。继续使用同一数据目录和钥匙串；目录同步不覆盖改过的来源，不自动启用新增预设。旧版移除来源导致范围不明确时，会保留不可用绑定并提示检查，避免自动扩大抓取范围。
...
- 1.3.0 只支持 RSS/Atom；普通网站首页不能直接当 RSS 使用，也没有隐藏的搜索引擎或网页爬虫。
